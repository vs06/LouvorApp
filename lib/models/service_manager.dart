...
class ServiceManager extends ChangeNotifier {
  ...
  List<Service> get filteredServices {
    ...
    if (search.isEmpty) {
      ...
      filteredServices.addAll(_allServices!.where((p) =>
          p.data.toString().toLowerCase().contains(search.toLowerCase())));
    }
  ...
  List<Service> filteredServicesByMonth(DateTime? dateTime) {
    ...
    filteredServices.addAll(_allServices!.where((service) =>
        ((service.data?.year == dateTime.year) &&
            (service.data?.month == dateTime.month))));

    filteredServices
        .sort((a, b) => a.data!.compareTo(b.data ?? DateTime.now()));
    ...
    if (_search != '') {
      ...
      filteredServices.forEach((service) {
        service.team?.forEach((key, volunteers) {
          volunteers.forEach((volunteer) {
            var volunteerWithinAccent =
                StringUtils.removeAccents(volunteer.toUpperCase());
            var searchWithinAccent =
                StringUtils.removeAccents(_search.toUpperCase());
            if (volunteerWithinAccent.contains(searchWithinAccent)) {
              if (!filteredServicesSearch
      ...
      return sortFilteredServices(
          filteredServicesSearch); //filteredServicesSearch;
  ...
  List<Service> sortFilteredServices(List<Service> lstService) {
    ...
    List<Service> beforeListService = lstService
        .where((element) => element.data!.isBefore(dateTime))
        .toList();
    List<Service> afterListService =
        lstService.where((element) => element.data!.isAfter(dateTime)).toList();
  ...
  Future<void> _loadAllServicesByDate(DateTime dateTime) async {
    ...
    QuerySnapshot snapServices = await fireStore
        .collection('services')
