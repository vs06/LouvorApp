...
class SongManager extends ChangeNotifier {
  ...
  List<Song> get filteredSongs {
    ...
    if (searchDTO.isfiltersEmpty()) {
      ...
      if (searchDTO.inactiveFilter) {
        filteredSongs.addAll(allInactiveSongs!
            .where((song) => (searchDTO.hasMatchWithSong(song))));
      } else {
        filteredSongs.addAll(
            allSongs!.where((song) => (searchDTO.hasMatchWithSong(song))));
      }
    }
  ...
  Future<void> _loadAllSongs() async {
    ...
    allSongs = snapSongs.docs.map((d) => Song.fromDocument(d)).toList();
  ...
  void update(Song song) {
    ...
    if (song.ativo?.toUpperCase() == 'FALSE') {
      ...
      int indexToBeUpdated =
          allInactiveSongs!.indexWhere((s) => s.id == song.id);
