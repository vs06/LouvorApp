...
class Rehearsal extends Service {
  ...
  Rehearsal.fromDocument(DocumentSnapshot document) {
    ...
    Map.from(document['lstSongs']).forEach((key, value) {
      if (lstSongs == null) lstSongs = [];
  ...
  Rehearsal clone() {
    return Rehearsal(
        id: id, type: type, data: data, isActive: isActive, lstSongs: lstSongs);
  }

  static Rehearsal serviceCastToRehearsal(
      Service service, String rehearsalType) {
    return Rehearsal(
        id: service.id,
        type: rehearsalType,
        data: service.data,
